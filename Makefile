...
test:
	for component in $(COMPONENTS); do \
	  (cd examples/$$component/ && \
	    make -f ../../src/$(MAKEFILE_NAME) deps ci deps-upgrade update-dotfiles update-partials update-to-main && \
	    make -f ../../src/$(MAKEFILE_NAME) update-to-version $(TARGET_VERSION_VARIABLE)=0.1.0 &&\
